...
namespace HL_PhysicsEngine
{
	...
	public:
		...
		Box(PxVec3 position = PxVec3(0.f, 0.f, 0.f), PxVec3 rotation = PxVec3(0.f, 0.f, 0.f), PxVec3 dimensions = PxVec3(.5f, .5f, .5f), PxReal density = PxReal(1.f)) :
			DynamicActor(PxTransform(position, PxQuat((rotation.magnitude()/360)*2*PxPi, rotation.getNormalized())))
		{
			CreateShape(PxBoxGeometry(dimensions), density);
		}
	};
