...
namespace HL_PhysicsEngine
{
	...
	public:
		PyramidStatic(PxTransform pose=PxTransform(PxIdentity)) :
			TriangleMesh(vector<PxVec3>(begin(wedge_verts),end(wedge_verts)), vector<PxU32>(begin(pyramid_trigs),end(pyramid_trigs)), pose)
		{
		}
	};
	...
	public:
		...
		void SetVisualisation()
		{
			...
			px_scene->setVisualizationParameter(PxVisualizationParameter::eACTOR_AXES, 1.2f);
		}
		...
		{
			SetVisualisation();			
			camera = VisualDebugger::GetCamera();

			GetMaterial()->setDynamicFriction(1.f);
			GetMaterial()->setStaticFriction(1.f);
			GetMaterial()->setRestitution(0.f);

			plane = new Plane();
			plane->Color(138.f,227.f,164.f);
			Add(plane);
			...
			/*for (int i = 0; i < 1000; i++)
{
	...
	this->Add(sphere);
			...
			HL_StaticEnclosure* statEnc = new HL_StaticEnclosure(10, 5, 10, 0.25,PxTransform(PxIdentity));
		...
		virtual void CustomUpdate() 
		{
			...
			if (VisualDebugger::key_state['P'])
			{
				PxVec3 bulletPos = camera->getEye();
				//PxVec3 bulletDirection = camera->get
			}
			//PxShape* shape = capsuleCompound->GetShape(1);
			//PxTransform oldTransform = shape->getLocalPose();
			//PxQuat newRotation = PxQuat(0.f,0.2f,0.f,0.f)* oldTransform.q;
			//shape->setLocalPose(PxTransform(oldTransform.p, (shape->getLocalPose().q * PxQuat(((2*PxPi)/600.0f),PxVec3(0.f,1.f,0.f)).getNormalized())));

			//shape = capsuleCompound->GetShape(2);
			//oldTransform = shape->getLocalPose();
			//newRotation = PxQuat(0.f, 0.2f, 0.f, 0.f) * oldTransform.q;
			//shape->setLocalPose(PxTransform(oldTransform.p, (shape->getLocalPose().q * PxQuat(((2 * PxPi) / 600.0f), PxVec3(0.f, .0f, 1.f)).getNormalized())));


			//shape = capsuleCompound->GetShape(3);
			//oldTransform = shape->getLocalPose();
			//newRotation = PxQuat(0.f, 0.2f, 0.f, 0.f) * oldTransform.q;
			//shape->setLocalPose(PxTransform(oldTransform.p, (shape->getLocalPose().q * PxQuat(((2 * PxPi) / 600.0f), PxVec3(1.f, .0f, .3f)).getNormalized())));

			/*shape = capsuleCompound->GetShape(0);
			oldTransform = shape->getLocalPose();
			newRotation = 
			shape->setLocalPose(PxTransform(oldTransform.p, (shape->getLocalPose().q * PxQuat(((2 * PxPi) / 60.0f), PxVec3(0.f, .5f, 1.0f)).getNormalized())));*/

			//for each(PxShape* shape in capsuleCompound->GetShapes())
			//{
			//	shape->setLocalPose(PxTransform(shape->getLocalPose().p, shape->getLocalPose().q+ PxQuat((2 * PxPi) / (1.f / 60.f), PxVec3(0.f, 1.f, 0.f))));
