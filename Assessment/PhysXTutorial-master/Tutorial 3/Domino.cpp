...
namespace HL_PhysicsEngine {
	HL_Domino::HL_Domino(PxTransform& pose, PxVec3 dims) :Box(pose, dims, dominoDensity)
	{
		PxMaterial* mat = GetMaterial(HL_Materials::plastic);
		GetShape()->setMaterials(& mat,1);
	}

	HL_DominoContainer::HL_DominoContainer(PxVec3 start, PxVec3 end, PxVec3 domDims)
	{
		...
		int numDominos = distance / (domDims.z + (domDims.y));
		...
		for (int i = 0; i <= numDominos; i++)
		{
			...
			HL_Domino* newDomino = new HL_Domino(PxTransform(start + (distanceIncrement * i)),domDims);
	...
	HL_Image::HL_Image(PxVec3 position, PxVec3 dimensions, PxVec3 domDims)
	{
		...
		for (int i = 0; i < numRows - 1; i++)
		{
			...
			Box* turner = new Box(PxTransform(position.x + xpos, position.y + domDims.y, zpos),PxVec3(domDims.z/5,domDims.y/5, domDims.x*1.8f),.8f);
			turner->Color(PxVec3(160.f / 225.f, 196.f / 255.f, 226.f / 255.f));
